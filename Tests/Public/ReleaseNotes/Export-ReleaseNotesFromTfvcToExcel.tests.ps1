...
Describe 'Export-ReleaseNotesFromTfvcToExcel' {
    BeforeAll {
        ...
        Mock -ModuleName $ModuleName -CommandName Get-WorkItemRefsListByChangeset -MockWith { @() }
    ...
    It 'Should call callect data using correct parameters' {
        # Act
        $result = Export-ReleaseNotesFromTfvcToExcel `
        ...
        Should -Invoke -ModuleName $ModuleName -CommandName Get-WorkItemRefsListByChangeset -Times 1 -ParameterFilter {
            $a = $CollectionUri -eq $expected.CollectionUri
            $b = $Project -eq $expected.Project
            $c = $TargetRepository -eq $expected.TargetRepository
            $d = $Branch -eq $expected.TrunkBranch
    ...
    It 'Should use default values when optional parameters are not provided' {
        ...
        # Assert
        Should -Invoke -ModuleName $ModuleName -CommandName Get-WorkItemRefsListByChangeset -Times 1 -ParameterFilter {
            $z = $Branch -eq $expected.TrunkBranch
            $z
        }
    ...
    It 'Should pass Show and PassThru parameters to Export-Excel' {
        # Act
        Export-ReleaseNotesFromTfvcToExcel -Show -PassThru
    ...
    It 'Should use UseConstantFileName when specified' {
        # Act
        Export-ReleaseNotesFromTfvcToExcel -UseConstantFileName
    ...
    It 'Should handle custom TimeZone' {
        ...
        # Act
        Export-ReleaseNotesFromTfvcToExcel -TimeZone $customTimeZone
