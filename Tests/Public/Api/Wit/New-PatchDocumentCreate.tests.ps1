...
Describe 'New-PatchDocumentCreate' {
    BeforeAll {
        Mock -ModuleName $ModuleName -CommandName Write-CustomProgress -MockWith { }
    }
    ...
    Context 'SourceWorkItem' {
        It 'Should create patch document with source work item and properties' {
            ...
            # Act
            $result = New-PatchDocumentCreate -SourceWorkItem $sourceWorkItem -Properties $properties
        ...
        It 'Should create patch document with additional data' {
            ...
            # Act
            $result = New-PatchDocumentCreate -SourceWorkItem $sourceWorkItem -Data $data
        ...
        It 'Should handle tags modifications' {
            # Arrange
            $sourceWorkItem = [PSCustomObject] @{
                url    = 'https://dev.azure.com/org/project/_apis/wit/workitems/123'
                rev    = 1
            ...
            # Act
            $result = New-PatchDocumentCreate `
        ...
        It 'Should handle null source work item' {
            # Act
            $result = New-PatchDocumentCreate -SourceWorkItem $null

            # Assert
            $result.Operations | Where-Object { $_.op -eq 'test' -and $_.path -eq '/rev' } | Should -BeNullOrEmpty
        }

        It 'Should combine all operation types' {
            # Arrange
            $sourceWorkItem = [PSCustomObject] @{ rev = 1; fields = [PSCustomObject] @{ field1 = 'value1' } }
            $properties = @('field1')
    ...
    Context 'SourceWorkItem Relations' {

        BeforeAll {
            $sourceParentWorkItemRelation = [PSCustomObject] @{
            ...
            $sourceChildWorkItemRelation = [PSCustomObject] @{
            ...
            $sourceWorkItem = [PSCustomObject] @{
                url       = 'https://dev.azure.com/org/project/_apis/wit/workitems/234'
                rev       = 1
                fields    = [PSCustomObject] @{
                    'System.WorkItemType' = 'Bug'
                }
                relations = @(
                    $sourceParentWorkItemRelation
                    $sourceChildWorkItemRelation
                )
            }
        }

        It 'Should work for AsChild' {
            # Act
            $result = New-PatchDocumentCreate `
                -SourceWorkItem $sourceWorkItem `
                -AsChild:$true
            ...
            # Assert
            $parentRelation = $result.Operations `
            | Where-Object { $_.op -eq 'add' -and $_.path -eq '/relations/-' } `
            | Where-Object { $_.value.rel -eq 'System.LinkTypes.Hierarchy-Reverse' }
            $parentRelation | Should -Not -BeNullOrEmpty -Because 'Parent relation should be added'
            $parentRelation.value.url `
            | Should -Be $sourceWorkItem.url -Because 'Parent point to source work item'
        }

        It 'Should work for not AsChild' {
            # Act
            $result = New-PatchDocumentCreate `
                -SourceWorkItem $sourceWorkItem `
                -CopyRelations:$true `
            ...
            $parentRelation | Should -Not -BeNullOrEmpty -Because 'Parent relation should be added'
            $parentRelation.value.url `
            | Should -Be $sourceParentWorkItemRelation.url -Because 'Parent point to source work item''s parent'
        }
    }
}
